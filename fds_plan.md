...
　　　Phase 6（擴充音源）尚未動工
　　　**7.4（背景垂直漂移）與 7.7（磁碟嗶聲）的症狀已隨 7.8 消失，
　　　但因果機制尚未驗證——見 7.10**
...
| gap / CRC | 完全不生成，只靠下降緣退 2 bytes | 對兩款測試遊戲夠用，不保證通用。**已於 7.11 踩到**：退磁頭時忘了同步退日誌長度 |
...
### 7.10 7.4 與 7.7 隨 7.8 一併消失 ✅（機制未驗證 ⚠️）

修掉 7.8 的位址解碼後實機確認：

- **7.4**《アルマナの奇跡》的背景垂直漂移 → 消失
- **7.7** 磁碟動作期間的間歇尖銳嗶聲 → 消失

**但只有跳躍音效那條有實證機制**（trace 裡直接看到 `$4020-$4023`
的值出現在方波 1 上）。`$4020→$4000` 的污染只涉及 APU，
**推不出視訊症狀**，所以 7.4 為何跟著好了，目前沒有經過驗證的解釋。
在補上證據之前，不要把「解碼修好」當成 7.4 的已知成因來引用。

**已排除的候選**：`ApuEventQueue[100]` 的 `cur_event++` 沒有邊界檢查，
且 map 顯示 `ChrBuf`（32 KB）就緊接在它後面，看起來像是完美的
「音訊寫入撐爆佇列 → 砸到繪圖緩衝」。但 `cur_event` **每條掃描線
歸零**（`InfoNES.cpp:732` 的 `InfoNES_pAPUHsync()`），而一條掃描線
只有 113 個 CPU cycle、最快的 store 4 cycle，最多約 28 筆——**撐不爆**。

**一個確定在發生、但尚未證實與 7.4 有因果的事**：

`$4040-$408F` 是 FDS 音源暫存器，`& 0x1f` 之後整段落在 APU 上。
其中 **`$4054` 與 `$4074` 落在 `0x14`，也就是精靈 DMA**——而這兩個位址
就在 FDS 波形 RAM（`$4040-$407F`）之內。遊戲每次載入波形表都會誤觸
一次 256 bytes 的 SPRRAM 搬移。這在修掉之前必定持續發生。

與 7.4 的觀察對得上嗎？**不完全對。** 7.4 明確記錄「精靈正常」，
而誤觸精靈 DMA 應該先弄壞精靈才對。所以這條也還不能算解釋。

**若要補證**：把 7.8 的修改單獨還原（保留 7.9 的 APU 修正），
在 `K6502_rw.h` 的 `case 0x14` 加一個計數器，看《アルマナの奇跡》
遊玩期間精靈 DMA 被誤觸幾次、以及漂移是否隨之回來。這是唯一能把
「相關」升級成「因果」的做法。


### 7.11 已解決：寫入日誌把 CRC bytes 一起收進去，蓋掉下一個區塊 ✅（2026-08-06）

**症狀**：《薩爾達傳說》磁碟版，輸入玩家名字之後出現 **ERR. 24**。

ERR 24 = BIOS 在該出現**檔案標頭區塊（block type `$03`）**的位置沒認出它。
22、23 是對 `$01`（disk info）與 `$02`（file amount）的同一種抱怨，
BIOS 依序讀、第一個失敗就停，所以走到 24 代表前兩塊都通過了。

**這次沒有用推理，直接量測**（`FDS_XFER_TRACE`，分支
`claude/fds-zelda-err24-trace`）。實機 log 的關鍵三行：

```
XFER 14 s=1 R st=    0 b=01 n=   58 en=   58 c=2d r=1
XFER 15 s=1 W st=   57 b=1d n=    8 en=   65 c=2d r=1 jofs=57 jlen=4
JOURNAL 0 side=1 ofs=   57 len=    4 dataofs=    0
```

`.fds` 的版面（由 log 中正常載入的部分反推確認）：

| 區塊 | 映像位置 | 大小 |
|---|---|---|
| `$01` disk info | 0 – 55 | 56 |
| `$02` file amount | 56 – 57 | 2 |
| **`$03` file header** | **58 – 73** | 16 |

**日誌項目 `ofs=57 len=4` 涵蓋 57、58、59、60 —— 58 正是 `$03` 的型別 byte。**
`Map20_SaveLookup()` 按範圍疊在映像上，於是 BIOS 每次讀 58 都拿到日誌內容
而不是 `$03`。錯誤碼的順序也對得上：`$01` 在 0–55 完全沒被碰到，
`$02` 的型別 byte 在 56 也沒被碰到（被污染的 57 是檔案數量），
所以前兩塊「通過」，第三塊才炸。

**成因**：`$4025` 起始位元下降時，磁頭退 2 bytes 補償映像裡不存在的 CRC
（7.5「gap / CRC」那列），**但 `FDS_SaveIndex[].dwLength` 沒有跟著退**。
程式碼知道最後 2 個 byte 是不存在的 CRC，卻只從磁頭扣掉、沒從日誌扣掉，
於是每一筆寫入日誌都比磁頭實際走過的範圍多 2 bytes，剛好溢進下一個區塊的開頭。

**修法**：`Map20_SaveTrimWrite(2)`，rewind 發生時同步把日誌項目縮短 2 bytes。
只有尾端項目會把 payload 空間還回去；中間被重用的項目保留配額，
那正是 `Map20_SaveBeginWrite()` 下次要長回去的空間。

**為什麼日誌壞掉之後重燒遊戲救不回來**：NVRAM 槽在
`NES_FILE_ADDR - SRAM_SIZE*(slot+1)`（`main.cpp:333`，本例 `0x7E000`），
燒錄從 `0x80000` 開始，**不在抹除範圍內**；加上 fingerprint 比對會認定
同一張磁片有效。所以寫壞一次就固定壞，這也是症狀看起來時有時無的原因。
要強制清除：先燒別的遊戲、再燒回來，fingerprint 對不上就會被丟棄。

**兩個沒收掉的線頭**（不影響上面的結論，但記著）：

- `n=8` 但 `jlen=4`：寫入 8 個 byte、日誌只記 4 個。`$4024` 裡
  `Map20_SaveWriteByte()` 與計數在同一個 `if` 內，`dwLimit` 也算得出 8048，
  **從程式碼推不出為什麼會差一半**。
- `st=57` 而不是 56：`$01` 讀 58 退 2 應該落在 56。差了 1。
  `case 0x4031` 只檢查 `FDS_DiskInserted`，**沒有檢查傳輸是否已武裝**，
  任何游離的暫存器讀取都會推進磁頭——可能是這裡，未驗證。

這兩點可能是同一件事。若日後又出現對齊類問題，從這裡查起。

---

*Phase 1–5 已實作完成並實機驗證；Phase 6 仍為計畫。*
*7.8、7.9 已解決並實機驗證。7.4、7.7 的症狀隨之消失，但**機制尚未驗證**（見 7.10）。*
*7.11 已定位並修正，機制由實機 log 直接證實；修法本身待實機驗證。*
