...
### 4.2 Ring buffer 寫入端無溢位保護，且會永久阻塞 ⚠️ 高（✅ **已修**，2026-08-06；正常路徑已實機驗證，防護路徑未驗證，見文末決議）
...
### 4.8 `$40xx` 位址解碼把匣帶暫存器折回 APU ⚠️ 高（✅ **已修**，2026-08-04）

`K6502_rw.h` 的 `case 0x4000:` 用 `switch (wAddr & 0x1f)` 解碼，
但只有 `$4000-$4017` 屬於 2A03，以上是匣帶空間。`0x1f` 遮罩讓
`$4020` 落在 `$4000`、`$4021` 落在 `$4001`，依此類推。

FDS 遊戲每幀重設 `$4020-$4022` 的間隔計時器（實測 60 次/秒），
於是**方波 1 每一幀都被砸掉一次**。另外 `$4034 & 0x1f = 0x14`
會誤觸**精靈 DMA**，`$4040-$408F` 整段落在 APU 上。

原本的 `MapperApu()` 仍有被呼叫，所以 mapper 功能本身正常——
污染是額外發生的，因此症狀是「聲音不對」而不是「遊戲不能玩」，
也讓它在這個 fork 裡潛伏至今。

**只有使用 `$4020` 以上的 mapper 會踩到**（FDS/mapper 20、部分轉版
mapper），NROM 等常見卡帶完全碰不到，這是它一直沒被發現的原因。

修法：`$4017` 以上一律交給 `MapperApu()`，不進 APU 解碼。
完整的解題過程、兩次推理錯誤與量測方法見 `fds_plan.md` 7.8。

修掉之後，`fds_plan.md` 7.4（背景垂直漂移）與 7.7（磁碟嗶聲）的症狀
也一併消失，但**只有音效那條有實證機制**，另外兩條的因果尚未驗證，
見 7.10。

### 4.9 pAPU 的三個既有缺陷 ⚠️ 中（✅ **已修**，2026-08-04）

查 4.8 的過程中順帶發現，與 4.8 無關：

- **sweep 單元缺靜音保護**（方波 1、2）：真機在目前週期 < 8 或目標
  > `$7FF` 時不寫回週期值。`ApuC?Freq` 宣告為 `DWORD`，上掃會下溢
  回捲成極大值，反而讓 `< 8` 的靜音判斷失效，聲道以凍結的相位增量
  持續發聲。
- **除零**：六處 `ApuPulseMagic / (Freq / 2)` 在 `Freq == 1` 時除數為 0。
  RP2040 硬體除法器**不會 fault**，只回垃圾值，所以是靜默錯誤。
- **緩衝區越界**：五個聲道的靜音路徑 `memset(…, 2 * n)`，但
  `wave_buffers[5][735]` 每列僅 735 bytes。n≈416 時越界 97 bytes，
  前四列被下個聲道蓋掉，DPCM 那列**直接寫出陣列之外**。
...
1. ~~**修 4.2（無限等待 + 無溢位保護）**~~
   已於 2026-08-06 實作並實機確認正常路徑無退化。
   防護路徑（截斷、逾時）正常遊玩時不會執行到，仍未驗證（見第 8 節決議）。
2. **加上 SPI 同步防護（4.4）**
...
> 4.8、4.9 已於 2026-08-04 修復並實機驗證，不列入待辦。
...
### 2026-08-06：4.2 已實作（⚠️ 未實機驗證）

**改動**（`main.cpp`，兩處）：

| 位置 | 改動 |
|---|---|
| `AudioRingBuffer::write()` | 回傳型別 `void` → `int`；在 spin lock 內重算剩餘空間並把 `len` 截斷，`head` 不再輾過 `tail` |
| `InfoNES_SoundOutput()` 的節流迴圈 | 加上 `AUDIO_THROTTLE_TIMEOUT_US`（50 ms）上限；只有在真的需要等待時才取 `time_us_64()` |

實作細節上與 4.2 節寫的十行修法有兩點差異：

- **剩餘空間在 lock 內重算，沒有呼叫 `writable_size()`。**
  `writable_size()` 自己不取 lock，在 `write()` 之前呼叫等於在鎖外取樣
  `head`／`tail`，截斷長度可能已經過期。改成在鎖內用同一份快照計算。
- **節流迴圈改成 `if (...) { do { ... } while (...); }`。**
  這樣正常路徑（緩衝未滿）完全不會呼叫 `time_us_64()`，
  維持 `__not_in_flash_func` 的原意。行為與原本的 `while` 迴圈等價。

`remaining -= n` 維持不變：`n` 是**送進** `write()` 的量而非被接受的量。
波形指標已經前進 `n`，若改成扣掉實際寫入量，緩衝滿時外層迴圈會永遠跑不完。

**這份改動在雲端工作階段推導，未經編譯。**
`AudioRingBuffer` 有抽出來在 x86 上以 `g++ -Wall -Wextra` 單獨編譯並測過
截斷與環繞；完整韌體的編譯與燒錄由使用者完成。

需要使用者實機確認的項目，以及 2026-08-06 的回報結果：

| # | 項目 | 結果 |
|---|---|---|
| 1 | **正常路徑沒有退化**——遊玩的聲音與速度與改動前一致。節流是速度節流器（見 4.1），若被動到，症狀會是遊戲變快或變慢 | ✅ 使用者回報「速度感覺一樣，聽不出差別」 |
| 2 | **逾時不會誤觸**——50 ms 遠大於正常等待時間，理論上遊玩中不該觸發；若聽到週期性掉音，表示 50 ms 太短或節流平衡被改變了 | ✅ 未聽到掉音 |
| 3 | **凍結是否真的不再發生** | ⚠️ **仍未驗證**，見下 |

第 3 項只有在原本會凍結的情境下才驗證得到，而**目前沒有可靠的重現步驟**。

**這次的測試結果能證明什麼、不能證明什麼**（重要，別過度解讀）：

- **能證明**：改動編譯得過、燒得進去，而且**正常路徑沒有被破壞**。
  考慮到 4.1 的取樣率是手工調出來的節流平衡，「聽不出差別」是有份量的結論——
  這正是這類改動最容易踩到的地雷，而它沒踩到。
- **不能證明**：修法本身有效。截斷與逾時**兩條路徑在正常遊玩時都不會被執行到**
  （緩衝不滿就不截斷，core1 沒卡住就不逾時）。也就是說，
  一份「改動根本沒被編進去」的韌體會給出**一模一樣的測試結果**。

因此 4.2 的狀態是「**正常路徑已驗證無退化，防護路徑未驗證**」。
防護路徑要驗證只能靠刻意注入故障（例如暫時把 `TARGET_LATENCY_BYTES` 調到極小
逼出截斷、或在 core1 迴圈裡塞人為停頓逼出逾時），目前判斷不值得為此冒險動
音訊路徑——這是防禦性修補，正常路徑不退化就已達成主要目的。

**關於觸發條件的補充**：上一則決議引用的觸發條件是 7.7 的磁碟嗶聲，
但該症狀已隨 [`fds_plan.md`](fds_plan.md) 7.8 的位址解碼修正一併消失，
且嗶聲的實證機制是 `$4020-$4023` 汙染方波 1，與 ring buffer 無關。
也就是說，**當初把 4.2 從「暫不處理」改列「待修」的那條證據，事後看是誤判**。
4.2 本身仍然值得修——它是唯一會造成整機凍結且無看門狗可救的路徑——
但不應再宣稱它與 7.7 有因果關係。
