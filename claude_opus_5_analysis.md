...
### 4.2 Ring buffer 寫入端無溢位保護，且會永久阻塞 ⚠️ 高（✅ **已實作**，2026-08-06，⚠️ **未實機驗證**，見文末決議）
...
1. ~~**修 4.2（無限等待 + 無溢位保護）**~~
   已於 2026-08-06 實作，**但尚未實機驗證**（見第 8 節決議）。
2. **加上 SPI 同步防護（4.4）**
...
**⚠️ 這份改動只在雲端工作階段推導，沒有編譯過、沒有燒錄過。**
需要使用者實機確認的項目：

1. **正常路徑沒有退化**——一般遊玩的聲音與速度與改動前一致。
   節流是速度節流器（見 4.1），若正常路徑被動到，症狀會是遊戲變快或變慢。
2. **逾時不會誤觸**——50 ms 遠大於正常等待時間，理論上遊玩中不該觸發；
   若聽到週期性掉音，表示 50 ms 太短或節流平衡被改變了。
3. **凍結是否真的不再發生**——這點只有在原本會凍結的情境下才驗證得到。
   目前**沒有可靠的重現步驟**，所以這條大概只能靠長時間遊玩累積信心。
